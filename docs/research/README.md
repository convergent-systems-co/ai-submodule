...
| **Orchestrator-Workers** | Central LLM delegates dynamically | Tech Lead → Coder delegation | Already used |
...
3. **No infrastructure exists.** Dark Forge deliberately removed MCP server code (commit b636ebd) with rationale "MCP configs belong in consuming repos." Zero skill files, zero awesome-dach-copilot references exist in the codebase.

4. **Two-repo coordination cost.** Splitting cognitive (dach-copilot) and enforcement (Dark Forge) layers creates version drift risk. Breaking changes to `panel-output.schema.json` require coordinated releases across repos.
