# CI and Branch Protection Blueprint -- Dark Forge
...
|-----------------------------------------------------|---------------------------------------------------|-----------------------------------------------------------------------------------------------|
| `Dark Forge / Panel Emissions Validation`         | Validates all panel structured JSON emissions.     | All emissions conform to schema. No missing required fields. Reasoning hashes match.          |
| `Dark Forge / Copilot Review Gate`                | Parses and evaluates Copilot review feedback.      | No unresolved critical/error findings. Confidence score meets profile threshold.              |
| `Dark Forge / Policy Evaluation`                  | Runs the policy engine and produces a decision.    | Policy engine returns a valid decision. No runtime errors.                                    |
| `Dark Forge / Merge Gate`                         | Final merge gating check based on policy decision. | Policy decision is `auto_merge`. All upstream checks passed. Run manifest is valid.           |
...
flowchart TD
    ...
    POLICY[Dark Forge /\nPolicy Evaluation] --> MERGE[Dark Forge /\nMerge Gate]
```
...
All four `Dark Forge` checks and the applicable `JM Compliance` checks must be configured as required status checks in the branch protection rules for every protected branch. See Section 5 for the branch protection rule template.
...
GitHub Copilot is designated as a formal review panel within the Dark Forge governance model. Its review feedback on pull requests is consumed programmatically, not treated as advisory.
...
| Policy Decision          | `Dark Forge / Merge Gate` State | GitHub Merge Behavior                     |
|--------------------------|-----------------------------------|-------------------------------------------|
...
2. All `Dark Forge` required status checks have passed.
...
branch_protection:
  ...
  required_status_checks:
    ...
    contexts:
      ...
      # Dark Forge governance checks
      - "Dark Forge / Panel Emissions Validation"
      - "Dark Forge / Copilot Review Gate"
      - "Dark Forge / Policy Evaluation"
      - "Dark Forge / Merge Gate"
...
jobs:
  ...
  # -----------------------------------------------------------
  panel-emissions-validation:
    name: "Dark Forge / Panel Emissions Validation"
  ...
  # -----------------------------------------------------------
  copilot-review-gate:
    name: "Dark Forge / Copilot Review Gate"
    runs-on: ubuntu-latest
    outputs:
  ...
  # -----------------------------------------------------------
  policy-evaluation:
    name: "Dark Forge / Policy Evaluation"
    runs-on: ubuntu-latest
    needs:
      - panel-emissions-validation
      - copilot-review-gate
