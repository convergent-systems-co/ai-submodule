...
TLDR for developers adopting the Dark Forge.
...
# Or skip this — the Tech Lead auto-generates project.yaml during /startup
```

Edit `project.yaml` to set your project name, language, and policy profile. If you skip this step, the Tech Lead will analyze your repo and create `project.yaml` automatically during the first `/startup` session. See [policy profiles](../architecture/governance-model.md) for guidance on which profile fits your use case.
...
5. **Phase 4 (Collect):** Wait for agents, run Test Evaluator evaluation (up to 3 FEEDBACK cycles) → report via `step --complete 4`
...
```bash
cp .ai/governance/templates/prompts/plan-template.md .artifacts/plans/42-my-feature.md
```
...
Emitted output (plans, panels, checkpoints, state) uses identical `.artifacts/` paths in both the Dark Forge repository and consuming repos. Read-only governance sources (personas, prompts, policies, schemas) differ — consumers access them via the `.ai/` submodule prefix.
...
```bash
ls -t .artifacts/checkpoints/*.json | head -1   # find the most recent checkpoint
cat .artifacts/checkpoints/<latest>.json         # see where things left off
```

Then tell the agent: `continue` or `Resume from checkpoint: .artifacts/checkpoints/<file>`. See [checkpoint resumption workflow](../../governance/prompts/checkpoint-resumption-workflow.md) for the full protocol.
...
| Recent checkpoints | `ls -t .artifacts/checkpoints/*.json \| head -5` |
...
| `/checkpoint` | Save session state and stop | Writes a checkpoint to `.artifacts/checkpoints/` with current progress, remaining work, and git state. Use when context capacity is high or you need to pause. Follow with `/clear` to reset context. |
| `/threat-model` | Run threat model analysis | Modes: `/threat-model` (current PR), `/threat-model system` (full platform), `/threat-model pr=N` (specific PR). Outputs markdown and structured JSON to `.artifacts/panels/`. |
...
- [governance/personas/agentic/](../../governance/personas/agentic/) — 7 agentic personas (Project Manager, DevOps Engineer, Tech Lead, Coder, IaC Engineer, Test Evaluator, Document Writer)
