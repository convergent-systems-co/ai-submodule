...
| **Plans as Audit Trail** | Every decision is written to `.artifacts/plans/`. Team members can see exactly why a change was made and how it evolved. |
...
| **GitHub access** | Needed to clone the submodule. Use SSH: `git@github.com:convergent-systems-co/dark-forge.git` |
...
Run the init script. It creates symlinks for all AI tools, sets up `.artifacts/plans/` and `.artifacts/panels/` folders, copies governance workflows, and generates CODEOWNERS.
...
Copy a template for your stack, or skip this — the Tech Lead will auto-detect your codebase and generate `project.yaml` during the first agentic session.
...
!!! note "What happens next"
    Five AI personas activate: DevOps Engineer (triage) → Tech Lead (plan) → Coder (implement) → Test Evaluator (evaluate) → Security Review → Merge. If no `project.yaml` exists, the Tech Lead creates one first.
...
| `.artifacts/plans/` / `.artifacts/checkpoints/` | New folders for audit trail and session state. | **Low** |
...
=== "Local Mode"
    ...
    - Plans written to `.artifacts/plans/`
...
|---------|-------------|
| "Build me a theme switcher" | DevOps Engineer triages → Tech Lead plans → Coder implements → Test Evaluator evaluates → Security review → merge |
...
| `/startup` | DevOps Engineer scans open PRs → triages issues → Tech Lead orchestrates → Coder + Tester loop → merge |
...
```powershell
# Pull latest Dark Forge updates
...
| Don't know where you are | `git status` · `git branch --show-current` · `ls .artifacts/checkpoints/` |
