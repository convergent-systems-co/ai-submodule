...
| Current plan | `.artifacts/plans/{active-plan}.md` | Session start if plan exists |
...
No shared mutable state between agents. Results are aggregated by the Tech Lead after all agents complete.
...
2. **Checkpoint handoff**: In the new thread, paste: "Resume from checkpoint: `.artifacts/checkpoints/{file}`" so the agent reads the checkpoint and continues.
...
3. **Write a checkpoint** to `.artifacts/checkpoints/{timestamp}-{branch}.json`:
   ```json
   {
...
Session N (context pressure detected)
  └─ Shutdown Protocol writes checkpoint to .artifacts/checkpoints/
...
Session N+1 (after context reset)
  ...
  └─ Phase 0: Checkpoint Auto-Recovery
       ├─ Scans .artifacts/checkpoints/ for most recent JSON file
...
|----------|-------------------|---------------------|----------------|
| **Claude Code** | Run `/clear` | Run `/startup` | Phase 0 scans `.artifacts/checkpoints/` automatically |
| **GitHub Copilot** | Start a new chat thread | Paste: "Resume from checkpoint: `.artifacts/checkpoints/{file}`" | Phase 0 reads the referenced checkpoint file |
| **CLI / Other** | Start a new session | Run `/startup` or equivalent | Phase 0 scans `.artifacts/checkpoints/` automatically |
...
Checkpoint files use the naming convention `{timestamp}-{branch}.json` and are stored in `.artifacts/checkpoints/`. The `ls -t` scan in Phase 0 ensures the most recent checkpoint is always selected.
...
2. **Checkpoint artifacts**: At each workflow gate, the agent writes a checkpoint file to `.artifacts/checkpoints/`. Contains: current state, decisions made, findings so far. If context resets, the agent reads the checkpoint to resume.
