...
The LLM calls the CLI between each phase. State is persisted to `.artifacts/state/sessions/`.
...
Sessions are persisted to `.artifacts/state/sessions/{session_id}.json` after every step. This is separate from checkpoints:
...
- **Max 3 Test Evaluator FEEDBACK cycles** per work unit — after 3 rounds of Tester feedback, the unit is blocked
