...
| Term | Definition |
|------|-----------|
| **Dark Forge** | An autonomous software delivery system where agentic processes handle the full lifecycle from intent to production, with policy-bound constraints replacing manual gates. |
...
| Tech Lead | `governance/personas/agentic/tech-lead.md` |
| Coder | `governance/personas/agentic/coder.md` |
| IaC Engineer | `governance/personas/agentic/iac-engineer.md` |
| Test Evaluator | `governance/personas/agentic/test-evaluator.md` |
...
**Primary:** Tech Lead persona

The Tech Lead persona is the enforcement authority for Layer 1. It validates incoming intents against the intent schema, checks for ambiguity, and routes validated packages to the appropriate workflow. The Tech Lead has the authority to:
...
Runtime Governance controls the system in production. It monitors deployed systems for drift, anomalies, performance degradation, and incidents that should trigger re-evaluation by the agentic system. This layer closes the feedback loop between production behavior and the governance pipeline, enabling the Dark Forge to respond to runtime conditions without waiting for human observation.
...
| Agent | Pattern | Role |
|-------|---------|------|
| Project Manager | Orchestrator-Workers | Portfolio orchestration, multiplexed Tech Leads (opt-in) |
| DevOps Engineer | Routing | Session lifecycle, pre-flight, issue triage, routing |
| Tech Lead | Orchestrator-Workers | Intent validation, panel selection, review coordination, merge |
| Coder | Worker | Implementation, test coverage, structured output |
| IaC Engineer | Worker | Infrastructure changes (conditional) |
| Test Evaluator | Evaluator-Optimizer | Independent evaluation, test verification, feedback |
...
| Routing | Emit ASSIGN messages to Tech Lead with issue context and priority |
...
- Communicate directly with Coder or Tester (all routing goes through Tech Lead)

See [devops-engineer.md](../../governance/personas/agentic/devops-engineer.md) for the full persona definition.

### 11.1 Tech Lead Persona -- Primary Orchestrator

The Tech Lead is the central orchestration authority in the Dark Forge. It operates across all five governance layers and is the only persona with system-wide coordination authority.

**Responsibilities by Layer:**

| Layer | Tech Lead Responsibility |
|-------|-----------------------------|
...
```mermaid
flowchart TD
    CM[Tech Lead<br/>Orchestrator] --> IV[Intent Validation]
...
The Coder is the primary execution agent. It operates under the direction of the Tech Lead and within the constraints established by Layers 1, 2, and 3.
...
| Plan Documentation | Produces implementation plans saved to `.artifacts/plans/` (consuming repos) or `governance/plans/` (Dark Forge repo) for review |
...
- Request clarification from the Tech Lead on ambiguous requirements
...
**Relationship to Tech Lead and Test Evaluator:**

```mermaid
flowchart TD
    CM[Tech Lead<br/>assigns work, monitors quality] -->|ASSIGN: DI + Plan| CO[Coder<br/>executes work, produces artifacts]
...
The Test Evaluator is the independent evaluator implementing Anthropic's **Evaluator-Optimizer** pattern. It assesses the Coder's output against acceptance criteria, test coverage requirements, and documentation standards. The Test Evaluator's approval is a mandatory gate before any review panels execute.
...
The Test Evaluator cannot:
- Modify code directly
- Be overridden by the Coder (the Test Evaluator's judgment is independent)
- Skip evaluation (every implementation must be evaluated)
- Communicate directly with DevOps Engineer

**Feedback Loop:** The Test Evaluator can send up to 3 rounds of FEEDBACK before the Tech Lead escalates to human review. This prevents infinite loops while ensuring quality.

See [test-evaluator.md](../../governance/personas/agentic/test-evaluator.md) for the full persona definition.
...
|----------|-------------|-------------|-------------|-------------|-------------|
| Accept/reject intent | Tech Lead | -- | -- | -- | -- |
| Select workflow | -- | Panel Graph + Tech Lead | -- | -- | -- |
| Assign personas | -- | Panel Graph + Tech Lead | -- | -- | -- |
| Approve code changes | -- | -- | Policy Engine | -- | -- |
| Merge to main | -- | -- | Policy Engine OR Human | -- | -- |
| Deploy to production | -- | -- | Policy Engine + CI | -- | -- |
| Generate remediation DI | -- | -- | -- | Tech Lead (automated) | -- |
...
| Delegation without substance | "Per Tech Lead direction" | Must include the Tech Lead's reasoning |
...
**Tier 1** activates when the policy engine script exists at `${GOV_ROOT}/governance/bin/policy-engine.py`. This is the case for the `dark-forge` repository itself and for consuming repos where the `.ai/` submodule content is fully checked out in CI.
...
| **L1: Intent** | Tech Lead (validation, routing) | Product Manager (requirements) |
| **L2: Cognitive** | Tech Lead (panel selection, orchestration) | Architect, Tech Lead (via review prompts) |
| **L3: Execution** | Coder (implementation), Tester (evaluation) | All panel participants (21 panels), Policy Engine |
| **L4: Runtime** | Tech Lead (DI generation) | Incident Commander, SRE, Observability Engineer |
| **L5: Evolution** | Tech Lead (version management) | Refactor Specialist, Systems Architect, Tech Lead, Minimalist Engineer |

### A.3 Decision Flow for a Feature Request

```mermaid
flowchart TD
    FR[Feature Request Enters System] --> L1{L1: Tech Lead<br/>validates intent}
    L1 -->|ACCEPT| L2[L2: Tech Lead selects panels<br/>based on codebase type]
...
| `governance/plans/` (Dark Forge repo) / `.artifacts/plans/` (consuming repos) | Cognitive | Implementation plans |
