...
This document describes the cross-repository issue escalation system — a mechanism for consuming repositories to automatically detect framework-level issues in the `.ai` governance submodule and escalate them to the upstream [Dark Forge](https://github.com/convergent-systems-co/dark-forge) repository.
...
3. **Search before creating** — `gh search issues --repo convergent-systems-co/dark-forge "dedup:{key}" --state open`
...
escalation:
  enabled: true
  target_repository: "convergent-systems-co/dark-forge"  # default
...
1. Create a fine-grained PAT at `github.com/settings/tokens`:
   - **Repository access:** Select `convergent-systems-co/dark-forge`
...
```json
{
  ...
  "target_repository": "convergent-systems-co/dark-forge",
  "detection_criteria": {
...
Records are saved to `.artifacts/panels/escalation-{id}.json` in the consuming repo (ephemeral, not committed).

## Upstream Issue Format

Escalated issues on the Dark Forge repository (`convergent-systems-co/dark-forge`) follow this format:
