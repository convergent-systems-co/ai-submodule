...
describe("buildServerEntry", () => {
  it("returns npx entry when not local", () => {
    ...
    expect(entry.args.some(a => a.includes("@jm-packages/ai-submodule-mcp"))).toBe(true);
  });
...
describe("mergeServerConfig", () => {
  ...
  const testEntry = { command: "npx", args: ["-y", "@jm-packages/ai-submodule-mcp@latest"] };
  ...
  it("creates new config file if missing", () => {
    ...
    expect(config.mcpServers["ai-submodule-mcp"]).toEqual(testEntry);
  });
  ...
  it("handles dot-separated keys (mcp.servers)", () => {
    ...
    const config = JSON.parse(readFileSync(fp, "utf-8"));
    expect(config.mcp.servers["ai-submodule-mcp"]).toEqual(testEntry);
  });
  ...
  it("is idempotent — safe to run multiple times", () => {
    ...
    expect(Object.keys(config.mcpServers)).toEqual(["ai-submodule-mcp"]);
  });
...
describe("verifyConfig", () => {
  ...
  it("returns true when server entry exists", () => {
    ...
    writeFileSync(fp, JSON.stringify({
      mcpServers: { "ai-submodule-mcp": { command: "npx", args: [] } },
    }));
  ...
  it("handles dot-separated keys", () => {
    ...
    writeFileSync(fp, JSON.stringify({
      mcp: { servers: { "ai-submodule-mcp": { command: "npx" } } },
    }));
