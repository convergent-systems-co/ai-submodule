...
export const DEFAULTS = {
  CACHE_DIR: join(homedir(), ".dark-forge-mcp", "cache"),
...
export async function setCachedData(
  ...
  try {
    ...
    console.error(`[dark-forge-mcp] Warning: failed to write cache for key "${key}"`);
  }
}
...
export async function loadCatalog(
  ...
  if (catalogUrl && !isOfflineMode()) {
    try {
      ...
      console.error("[dark-forge-mcp] Remote catalog fetch failed, trying cache...");
    }
  }
  ...
  if (cached) {
    ...
    if (options?.validateHash) {
      ...
      if (hash !== options.validateHash) {
        ...
        console.error(`[dark-forge-mcp] Cache hash mismatch for "${cacheKey}", re-fetching...`);
      } else {
        return cached.data;
      }
    } else {
