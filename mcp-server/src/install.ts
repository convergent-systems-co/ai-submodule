/**
 * install.ts — Multi-IDE auto-installer for dark-forge-mcp.
 *
...
const SERVER_NAME = "dark-forge-mcp";
const PACKAGE_NAME = "@jm-packages/dark-forge-mcp";
...
export function printHelp(): void {
  const help = `
dark-forge-mcp install — Multi-IDE MCP server auto-installer

Usage:
  dark-forge-mcp install [options]
...
Examples:
  dark-forge-mcp install                    # Auto-detect and configure all IDEs
  dark-forge-mcp install --dry-run          # Preview changes
  dark-forge-mcp install --ide vscode       # Configure VS Code only
  dark-forge-mcp install --local            # Use local path instead of npx
`.trim();
  console.log(help);
}
...
export async function runInstaller(argv: string[]): Promise<void> {
  ...
  console.log("=== dark-forge-mcp installer ===\n");
