...
Make the Dark Forge governance workflow function identically in consuming repos (where files are under `.ai/`) and in the Dark Forge repo itself (where files are at the root). Fix SSH URL issue for CI compatibility.
...
|------|-----------|--------|------------|
| Workflow change breaks Dark Forge CI | Low | High | Test root-relative paths still work |
