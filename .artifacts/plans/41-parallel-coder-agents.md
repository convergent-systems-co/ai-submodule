...
**Date:** 2026-02-25
**Status:** approved
**Issue:** https://github.com/convergent-systems-co/dark-forge/issues/41
...
Replace the sequential one-issue-at-a-time startup loop with parallel execution where the Team Lead spawns up to 5 concurrent Coder agents, each working on a different issue in an isolated git worktree. This eliminates the bottleneck of serializing independent issues.
...
Claude Code's `Task` tool with `isolation: "worktree"` provides exactly this capability — each subagent gets its own copy of the repo, its own branch, and its own context window. The Team Lead orchestrates from the main session.
...
| `governance/personas/agentic/team-lead.md` | Add parallel dispatch responsibilities, worktree coordination |
...
1. **Update `startup.md`**:
   ...
   - Phase 2 becomes "Parallel Planning" — Team Lead validates and plans all issues (up to session cap)
...
| 2026-02-25 | Team Lead stays in main session, Coders in worktrees | Team Lead needs to orchestrate and monitor; Coders can be fully autonomous |
