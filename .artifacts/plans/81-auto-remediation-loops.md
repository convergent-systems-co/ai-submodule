...
**Author:** Team Lead (agentic) / Coder (agentic)
**Date:** 2026-02-21
**Status:** approved
**Issue:** https://github.com/convergent-systems-co/dark-forge/issues/81
...
Add the governance artifacts required for autonomous drift remediation: two JSON schemas (remediation action record and verification result) and an agentic workflow prompt that guides the Team Lead through the remediation decision tree. This completes the Phase 4b "auto-remediation loops" capability at the governance artifact layer.
...
| `governance/prompts/remediation-workflow.md` | Agentic workflow prompt for the Team Lead to execute remediation loops |
...
3. **Create `remediation-workflow.md`** — An agentic prompt (similar in structure to `startup.md`) that the Team Lead follows when a drift signal triggers remediation. Steps: receive drift signal → classify severity → check policy thresholds → check circuit breaker → select remediation action → execute → verify → capture new baseline or revert → audit.
