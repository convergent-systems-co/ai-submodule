...
The Tech Lead routes work to the IaC Engineer (instead of the Coder) when:
...
- **Receive ASSIGN messages from Tech Lead** — accept infrastructure tasks with plan references, scope constraints, and acceptance criteria
- Create feature branches following the repository's branch naming convention
- Write a detailed implementation plan to `.artifacts/plans/` before writing infrastructure code
...
- **Emit structured RESULT to Tech Lead** — report completion with summary, artifacts, and validation results
...
- If `getResourceNames()` does not produce a name for a required resource type, ESCALATE to Tech Lead
...
Always prefer registry modules over inline resource definitions. If a needed module does not exist in the registry, ESCALATE to Tech Lead with a request to create one.
...
| New module creation | None — ESCALATE to Tech Lead with module request |
...
<!-- AGENT_MSG_START -->
{
  ...
  "payload": {
    "summary": "Implemented infrastructure for X per plan .artifacts/plans/{N}-description.md",
...
```mermaid
flowchart TD
    A[Tech Lead] -->|ASSIGN: infra task, plan, constraints| B[IaC Engineer: Plan, Implement, Validate]
