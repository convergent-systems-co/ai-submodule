...
# --- Output library ---
# Source the centralized output library for verbosity-aware logging.
_COMMON_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [ -f "$_COMMON_LIB_DIR/output.sh" ]; then
  source "$_COMMON_LIB_DIR/output.sh"
fi

# --- Logging (backward-compatible wrappers) ---
log_ok()    { out_step_ok "$*"; }
log_warn()  { out_warn "$*"; }
log_error() { out_error "$*"; }
log_skip()  { out_step_skip "$*"; }
log_info()  { out_verbose "$*"; }
log_debug() { out_debug "$*"; }
...
copy_with_diff() {
  ...
  if [ -L "$dst" ]; then
    if [ "$DRY_RUN" = "true" ]; then
      out_verbose "[DRY-RUN] Would replace symlink with copy: $dst"
      return 0
    fi
    rm "$dst"
    out_verbose "[MIGRATE] Replacing symlink with copy: $dst"
  fi
  if [ ! -f "$dst" ]; then
    if [ "$DRY_RUN" = "true" ]; then
      out_verbose "[DRY-RUN] Would copy: $src -> $dst"
      return 0
    fi
    cp "$src" "$dst"
    out_verbose "[COPY] $dst"
  elif ! diff -q "$src" "$dst" >/dev/null 2>&1; then
    if [ "$DRY_RUN" = "true" ]; then
      out_verbose "[DRY-RUN] Would update (content differs): $dst"
      return 0
    fi
    cp "$src" "$dst"
    out_verbose "[UPDATE] $dst"
  else
    out_debug "[SKIP] $dst (unchanged)"
  fi
}
...
sync_dir_with_diff() {
  ...
  if [ -L "$dst_dir" ]; then
    if [ "$DRY_RUN" = "true" ]; then
      out_verbose "[DRY-RUN] Would replace directory symlink with copy: $dst_dir"
      for src_file in "$src_dir"/*; do
        [ -f "$src_file" ] || continue
        local fname
        fname=$(basename "$src_file")
        out_verbose "[DRY-RUN] Would copy: $src_file -> $dst_dir/$fname"
      done
      return 0
    fi
    rm "$dst_dir"
    mkdir -p "$dst_dir"
    out_verbose "[MIGRATE] Replacing directory symlink with copy: $dst_dir"
  else
    mkdir -p "$dst_dir"
  fi
  for src_file in "$src_dir"/*; do
