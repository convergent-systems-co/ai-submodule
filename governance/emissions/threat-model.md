...
```mermaid
graph TD
    subgraph REPO["Dark Forge repository"]
...
| T3 | Confusion between governance/templates/ and governance/templates/prompts/ | N/A (operational risk) | Low | Negligible | **Negligible** |
...
**Attack vector:** Developer or automated tooling confuses `governance/templates/` (language scaffolding) with `governance/templates/prompts/` (cognitive prompt templates).
...
- `governance/templates/prompts/` contains `plan-template.md`, `runtime-di-template.md`, `weekly-report-template.md`.
...
1. **What if `governance/templates/` already exists?** — It did not. `governance/templates/prompts/` exists but is a different path. `git mv` would fail if the target existed — it succeeded, confirming no collision.

2. **What if init.sh runs inside the Dark Forge repo itself (not a consuming repo)?** — The GOALS template copy is guarded by `if [ "$IS_SUBMODULE" = "true" ]` (line 202). When running inside the submodule repo directly, this block is skipped entirely. No path dependency.
...
The change **improves architectural consistency** by eliminating the last top-level exception. The separation between `governance/templates/` (language conventions) and `governance/templates/prompts/` (prompt templates) is clear and well-documented. No new coupling introduced.
