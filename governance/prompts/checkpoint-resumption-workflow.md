...
- A checkpoint file exists in `.artifacts/checkpoints/`, written by the shutdown protocol
...
```bash
ls -t .artifacts/checkpoints/*.json | head -1
```
...
2. **Re-read the plan** (if resuming mid-implementation):
   ```bash
   cat .artifacts/plans/<issue-number>-*.md
   ```
...
1. Check if `.artifacts/checkpoints/` contains recent files (< 24 hours)
