...
This checklist defines the required governance steps for every PR in the Dark Forge pipeline, and how to verify each step was completed. The governance-compliance-review panel evaluates PRs against this checklist.

## Required Steps

### 1. Plan Created (Phase 2d of startup.md)

**Requirement:** Every PR must have an implementation plan in `.artifacts/plans/` created before implementation began.

**Verification:**
- Check if a file matching `.artifacts/plans/{issue-number}-*.md` exists in the PR diff or on the branch
