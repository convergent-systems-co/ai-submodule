...
class AuditLog:
    ...
    # ------------------------------------------------------------------
    # Convenience methods for common orchestrator events
    # ------------------------------------------------------------------
    ...
    def log_phase_transition(
        ...
        Args:
            ...
            action: Gate action taken (proceed/skip_dispatch/checkpoint/emergency_stop).
            correlation_id: Optional work-unit correlation id.
            detail: Additional structured detail.
        """
        event_detail = {"from_phase": from_phase, "to_phase": to_phase}
        if detail:
    ...
    def log_signal(
        ...
        """Log a capacity signal recording event.

        Args:
            session_id: Current session identifier.
            phase: Current phase when the signal was recorded.
            signal_type: Signal kind (tool_call/turn/issue_completed).
            count: Number of signals recorded.
            tier: Resulting capacity tier after recording.
            detail: Additional structured detail.
        """
        event_detail = {"signal_type": signal_type, "count": count}
        if detail:
            event_detail.update(detail)
        self.record(AuditEvent(
    ...
    def log_dispatch(
        ...
        Args:
            session_id: Current session identifier.
            phase: Current phase (typically 3).
            task_ids: List of dispatched task identifiers.
            persona: Agent persona being dispatched.
            correlation_id: Optional work-unit correlation id.
            detail: Additional structured detail.
        """
        event_detail = {"task_ids": task_ids, "task_count": len(task_ids)}
        if persona:
            event_detail["persona"] = persona
        if detail:
            event_detail.update(detail)
        self.record(AuditEvent(
            event_type="dispatch",
    ...
    def log_gate_check(
        ...
        Args:
            session_id: Current session identifier.
            phase: Phase being gate-checked.
            tier: Current capacity tier.
            action: Gate action (proceed/skip_dispatch/checkpoint/emergency_stop).
            would_shutdown: Whether this gate check would trigger shutdown.
            detail: Additional structured detail.
        """
        event_detail = {"would_shutdown": would_shutdown}
        if detail:
            event_detail.update(detail)
        self.record(AuditEvent(
