...
class TestPersonaDetection:
    ...
    def test_test_writer_branch(self, hook):
        assert hook.detect_persona("itsfwcp/test-writer/42/add-tests") == "test-writer"

    def test_test_evaluator_branch(self, hook):
        assert hook.detect_persona("itsfwcp/test-evaluator/42/eval-tests") == "test-evaluator"

    def test_tech_lead_branch(self, hook):
        # Tech leads are not restricted by the mechanical hook
