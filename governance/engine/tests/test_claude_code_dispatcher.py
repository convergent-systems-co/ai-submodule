...
class TestDispatch:
    ...
    def test_dispatch_sets_persona_path(self, dispatcher):
        tasks = [_make_task(persona=AgentPersona.TEST_EVALUATOR)]
        task_ids = dispatcher.dispatch(tasks)
        inst = dispatcher.all_instructions[task_ids[0]]
        assert inst.persona_path == _PERSONA_PATHS[AgentPersona.TEST_EVALUATOR]

    def test_dispatch_sets_plan_path(self, dispatcher):
        tasks = [_make_task()]
        task_ids = dispatcher.dispatch(tasks)
        inst = dispatcher.all_instructions[task_ids[0]]
        assert inst.plan_path == ".artifacts/plans/issue-42.md"
