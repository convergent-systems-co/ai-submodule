#!/usr/bin/env python3
"""Dark Forge Policy Engine — Phase 4b Runtime.
...
def _evaluate_block_condition(condition: str, confidence: float, risk: str, flags: list, emissions=None) -> bool:
    ...
    # Pattern: copilot_findings_unresolved == true
    if cond == "copilot_findings_unresolved == true":
        if emissions:
            for e in emissions:
                if e.get("panel_name") == "copilot-review":
                    for f in e.get("findings", []):
                        counts = f.get("findings_count", {})
                        unresolved = counts.get("critical", 0) + counts.get("high", 0) + counts.get("medium", 0)
                        if unresolved > 0:
                            return True
        return False

    # Pattern: dependabot_alerts_open == true
    if cond == "dependabot_alerts_open == true":
        if emissions:
            for e in emissions:
                if e.get("panel_name") == "dependabot-review":
                    for f in e.get("findings", []):
                        counts = f.get("findings_count", {})
...
def evaluate_escalation_rules(aggregate_confidence, aggregate_risk, policy_flags, emissions, profile, log):
    ...
    # Check if any panel requires human review — only escalate when the
    # panel's own aggregate verdict is NOT approve (issue #693).
    for emission in emissions:
        if emission.get("requires_human_review", False):
            verdict = emission.get("aggregate_verdict", "")
            if verdict == "approve":
                log.record("escalation_panel_human_review", "pass",
                           f"Panel '{emission['panel_name']}' requires human review but verdict is approve — skipping escalation")
                continue
...
def _evaluate_auto_merge_condition(condition: str, confidence: float, risk: str, flags: list, emissions: list, ci_passed: bool) -> bool:
    ...
    # Only treat as a disqualifier when the panel's aggregate_verdict is not
    # "approve" — mirrors the verdict-aware guard in evaluate_escalation_rules
    # (issue #680, #693).
    if "requires_human_review" in cond and "false" in cond:
        return not any(
            e.get("requires_human_review", False) and e.get("aggregate_verdict", "") != "approve"
            for e in emissions
        )
...
def evaluate(emissions_dir, profile_path, ci_passed=True, commit_sha=None, pr_number=None, repo=None, log_stream=None, dry_run=False):
    ...
    log._stream.write(f"\n{'='*60}\n")
    log._stream.write(f"  Dark Forge Policy Engine v1.0.0\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Dark Forge Policy Engine — evaluate panel emissions against a policy profile."
    )
